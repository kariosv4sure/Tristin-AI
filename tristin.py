...
def chat(message):
    # === NEW: Check if bot should respond ===
    bot_info = bot.get_me()
    ...
    if message.chat.type == "private":
        ...
        # Group/supergroup/channel - only respond if mentioned or replied to
    ...
    # Prepare the message text (remove mention if in group)
    if message.chat.type != "private" and f"@{bot_username}" in text_lower:
        # Remove the bot mention from the message
        user_message = text_lower.replace(f"@{bot_username}", "").strip()
        if not user_message:  # If only mention with no text
