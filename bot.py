...
def get_conversation_key(user_id, chat_id=None):
    """Generate unique key for conversation context"""
    if chat_id:
        return f"{user_id}_{chat_id}"  # Group chat
    return str(user_id)  # Private chat
...
def ensure_user(uid):
    ...
    if uid not in users:
        users[uid] = {
            ...
            "last_interaction": datetime.now().isoformat()  # ADD THIS
        }
    users[uid]["messages"] += 1
    users[uid]["last_interaction"] = datetime.now().isoformat()  # ADD THIS
...
def verify_user(call):
    ...
    # === ADD THIS CHECK ===
    # If already verified, just acknowledge and return
    if uid in verified:
...
def chat(message):
    ...
    # Name triggers (no tag needed)
    trigger_keywords = [
        "tristin",
        "derieri",
        "miss tristin"
    ]
    keyword_called = any(word in text_lower for word in trigger_keywords)
    ...
    if message.chat.type == "private":
        ...
        # Ignore unless mentioned, replied to, or name is called
        if not (is_mentioned or is_reply_to_bot or keyword_called):
            return
    # === END CHECK ===
    ...
    if (text_lower.startswith(("define ", "translate ", "lyrics ")) or  # ADDED lyrics
    ...
    if message.chat.type != "private" and is_mentioned:
        user_message = text_lower.replace(f"@{bot_username}", "").strip()
        if not user_message:
            bot.reply_to(message, "Yes? I'm listening... 😏")
            return
    else:
        user_message = text

    # === CONVERSATION MEMORY SYSTEM ===
    # Get conversation key (unique per user/chat combination)
    if message.chat.type == "private":
        conv_key = str(message.from_user.id)  # Private chat
    else:
        conv_key = f"{message.from_user.id}_{message.chat.id}"  # Group chat
    ...
    conversation_history = conversations[conv_key][-4:] if len(conversations[conv_key]) > 4 else conversations[conv_key]
    ...
    system_prompt = (
        ...
        "Use the conversation history to remember context and continue naturally."
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(conversations[conv_key]) > 6:
                conversations[conv_key] = conversations[conv_key][-6:]
